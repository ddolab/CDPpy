...
@CDPpy_agent.tool
def get_VCD_profile(
    ctx: RunContext[dict], run_ids: list, cell_line: list, viability: str
) -> str:
    ...
    conc_filtered_by_id = (
        conc_filtered_by_cl
        if "All" in cleaned_run_ids
        else filter_data(conc_filtered_by_cl, "ID", cleaned_run_ids)
    )
    ...
    fig = px.line(
        df, x=x_axis, y="value", title="Viable Cell Concentration"
    )  # , color_discrete_sequence=color, line_dash_sequence=[line], symbol_sequence=[symbol])
    ...
    if viability == "on":
        ...
        )  # , line_dash_sequence=[line], symbol_sequence=[symbol])
        ...
        fig.update_layout(
            yaxis2={"side": "right", "title": "Viability (%)", "overlaying": "y"}
        )
...
def get_cell_profiles(
    ...
    if "VCC" in profiles:
        ...
        ivcc_filtered_by_id = (
            ivcc_filtered_by_cl
            if "All" in cleaned_run_ids
            else filter_data(ivcc_filtered_by_cl, "ID", cleaned_run_ids)
        )
        fig = px.line(
            ivcc_filtered_by_id,
            x=x_axis,
        ...
        cum_filtered_by_id = (
            cum_filtered_by_cl
            if "All" in cleaned_run_ids
            else filter_data(cum_filtered_by_cl, "ID", cleaned_run_ids)
        )
        fig = px.line(
            cum_filtered_by_id,
            x=x_axis,
        ...
        gr_filtered_by_id = (
            gr_filtered_by_cl
            if "All" in cleaned_run_ids
            else filter_data(gr_filtered_by_cl, "ID", cleaned_run_ids)
        )
        fig = px.line(
            gr_filtered_by_id,
            x=x_axis,
